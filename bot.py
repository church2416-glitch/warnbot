...
cur.execute('''CREATE TABLE IF NOT EXISTS settings (
    ...
    removal_log_channel_id INTEGER, -- 해제 기록 전용 채널 추가
    role_1_id INTEGER,
    role_2_id INTEGER,
    role_3_id INTEGER,
...
def get_guild_settings(guild_id):
    # 컬럼 순서 주의: log(0), removal(1), r1(2), r2(3), r3(4), admin(5)
...
@app_commands.describe(작업="확인 또는 신규설정", 제제채널="경고 부여 시 기록될 채널", 해제채널="자동만료/강제해제 기록될 채널")
...
async def setup_integrated(
    ...
    작업: str, 
    제제채널: discord.TextChannel = None, 
    해제채널: discord.TextChannel = None, 
    ...
    if 작업 == "check":
        ...
        embed.add_field(name="로그 채널 (제제)", value=f"<#{settings[0]}>" if settings[0] else "❌", inline=True)
        embed.add_field(name="로그 채널 (해제)", value=f"<#{settings[1]}>" if settings[1] else "❌", inline=True)
        ...
        if not (제제채널 and 해제채널 and 경고1단계 and 경고2단계 and 경고3단계 and 관리자역할):
            return await interaction.response.send_message("❌ 모든 항목을 입력해야 합니다 (제제채널, 해제채널 포함).", ephemeral=True)
        ...
        conn.commit()
        await interaction.response.send_message("✅ 제제/해제 기록 채널을 포함한 설정이 저장되었습니다.", ephemeral=True)
...
async def warn(interaction: discord.Interaction, 대상: discord.Member, 사유: str):
    ...
    settings = get_guild_settings(interaction.guild.id)
    if not settings or not settings[0]: return await interaction.response.send_message("❌ `/설정`에서 제제 채널을 먼저 등록해주세요.", ephemeral=True)
    ...
    async def select_callback(inter2: discord.Interaction):
        ...
        await inter2.response.edit_message(content=f"✅ {대상.mention} 경고 부여 (현재 **{current_count}**회)", view=None)
...
async def removewarn(interaction: discord.Interaction, 대상: discord.Member):
    ...
    async def select_callback(inter2: discord.Interaction):
        ...
        if settings and settings[1]: # removal_log_channel_id
            ...
            if log_ch:
                embed = discord.Embed(title="🗑️ 유저 경고 강제 해제", color=discord.Color.green())
        ...
        await inter2.response.edit_message(content=f"✅ 경고 해제 완료 (남은 경고: **{after_count}**회)", view=None)
...
async def reset_db(interaction: discord.Interaction):
    ...
    conn.commit()
    await interaction.response.send_message("✅ DB 초기화 및 테이블 재구성 완료.", ephemeral=True)
